 ...
 */
const extractArea = (text: string): number => {
 ...
 */
const extractZones = (text: string): ZoneConfig[] => {
  ...
  const addZoneIfMatched = (pattern: RegExp, zone: ZoneConfig): void => {
 ...
 */
const extractConstraints = (text: string): Constraints => {
  const lowerText = text.toLowerCase();
  ...
  return {
    ...
    low_traffic:
      lowerText.includes('low traffic') || lowerText.includes('low-traffic'),

    // High density constraint: true if text contains "high density" phrase
    high_density:
      lowerText.includes('high density') || lowerText.includes('high-density'),
  };
};
 ...
 */
export const parseText = (input: string): ParsedCityData => {
